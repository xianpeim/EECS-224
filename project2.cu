	...
	for(i=0;i<ROWS;i++){
		for(j=0;j<COLUMNS;j++){
			for(k=i-16;k<i+17;k++){
				for(l=j-16;l<j+17;l++){
					if(k==i&&l==j){
						sum -= (int)(*(input+k*COLUMNS+l) * 1088);
...
kernel0 (dtype *input, dtype *output)
{
  ...
  int idx = blockIdx.x * 16 + tidx;
  int idy = blockIdx.y * 16 + tidy;
  int id = idy * COLUMNS + idx;
  int tmpid, tmpthisid;
  //int flag = FILTER_SIZE;
  
  
	tmpthisid = tidy * 48 + tidx;
	tmpid = (idy-16) * COLUMNS + (idx-16);
	if( idx-16 >= 0 && idy-16 >= 0  && idx-16 < COLUMNS && idy-16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = tidy * 48 + tidx + 16;
	tmpid = (idy-16) * COLUMNS + idx;
	if( idx >= 0 && idy-16 >= 0  && idx < COLUMNS && idy-16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = tidy * 48 + tidx + 32;
	tmpid = (idy-16) * COLUMNS + (idx+16);
	if( idx+16 >= 0 && idy-16 >= 0  && idx+16 < COLUMNS && idy-16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+16) * 48 + tidx;
	tmpid = idy * COLUMNS + (idx-16);
	if( idx-16 >= 0 && idy >= 0  && idx-16 < COLUMNS && idy < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+16) * 48 + tidx + 16;
	tmpid = idy * COLUMNS + idx;
	if( idx >= 0 && idy >= 0  && idx < COLUMNS && idy < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+16) * 48 + tidx + 32;
	tmpid = idy * COLUMNS + (idx+16);
	if( idx+16 >= 0 && idy >= 0  && idx+16 < COLUMNS && idy < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+32) * 48 + tidx;
	tmpid = (idy+16) * COLUMNS + (idx-16);
	if( idx-16 >= 0 && idy+16 >= 0  && idx-16 < COLUMNS && idy+16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+32) * 48 + tidx + 16;
	tmpid = (idy+16) * COLUMNS + idx;
	if( idx >= 0 && idy+16 >= 0  && idx < COLUMNS && idy+16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
	
	tmpthisid = (tidy+32) * 48 + tidx + 32;
	tmpid = (idy+16) * COLUMNS + (idx+16);
	if( idx+16 >= 0 && idy+16 >= 0  && idx+16 < COLUMNS && idy+16 < ROWS) scratch[tmpthisid] = input[tmpid]; 
	else scratch[tmpthisid] = 0;
  ...
  __syncthreads ();
	...
	if(idx < COLUMNS && idy < ROWS){
		for(i=tidy;i<tidy+33;i++){
			for(j=tidx;j<tidx+33;j++){
				if(i==tidy+16&&j==tidx+16){
					tmp -= 1088 * scratch[i*48+j];
				}else{
					tmp += scratch[i*48+j];
				}
			}
...
main(int argc, char** argv)
{
  ...
  dim3 gb(COLUMNS/16+1, ROWS/16+1, 1);
  dim3 tb(16, 16, 1);
  ...
  ofile1 = "outputcpu33.raw";
  ofile2 = "outputgpu33.raw";
